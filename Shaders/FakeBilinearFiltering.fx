...
float4 UDLREdges(float2 texcoord)
{
	...
	int edgeCount = 0;
	[loop]
	for(float i=(1.0/BUFFER_HEIGHT),j=(1.0/BUFFER_WIDTH);j<=(1.0/BUFFER_WIDTH)*CyCles && edgeCount<4;i+=(1.0/BUFFER_HEIGHT)*DiamondSizeV,j+=(1.0/BUFFER_WIDTH)*DiamondSizeV)
	{
		if(NfoundU)
		{
			NextPixel=tex2Dlod(ReShade::BackBuffer,float4(texcoord.x  ,texcoord.y-i,0,0)).rgb;
			texcoord2.r=texcoord.y-i;
			NfoundU=(abs(CurrentPixel.r-NextPixel.r)+abs(CurrentPixel.g-NextPixel.g)+abs(CurrentPixel.b-NextPixel.b)<=(ThresoldDiffBetweenPixels/10));
			edgeCount+=!NfoundU;
		}
		if(NfoundD)
		{
			...
			NfoundD=(abs(CurrentPixel.r-NextPixel.r)+abs(CurrentPixel.g-NextPixel.g)+abs(CurrentPixel.b-NextPixel.b)<=(ThresoldDiffBetweenPixels/10));
			edgeCount+=!NfoundD;
		}
		if(NfoundL)
		{
			...
			NfoundL=(abs(CurrentPixel.r-NextPixel.r)+abs(CurrentPixel.g-NextPixel.g)+abs(CurrentPixel.b-NextPixel.b)<=(ThresoldDiffBetweenPixels/10));
			edgeCount+=!NfoundL;
		}
		if(NfoundR)
		{
			...
			NfoundR=(abs(CurrentPixel.r-NextPixel.r)+abs(CurrentPixel.g-NextPixel.g)+abs(CurrentPixel.b-NextPixel.b)<=(ThresoldDiffBetweenPixels/10));
			edgeCount+=!NfoundR;
		}
	}
	return texcoord2;
...
technique FakeBilinearFiltering<	ui_label = "Fake Bilinear Filtering";>
{
	pass
    {
